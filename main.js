...
global.authFile = 'AyanokoyiSession'
global.authFileJB  = 'AyanokoyiJadiBot'
...
│ ${chalk.blueBright('╰┄┈┅┈┄┈┅┈┄┅┈┄┈┅┄┈┅┈┄')} 
│ ${chalk.italic.red(`Ayanokoyi Bot - MD`)}
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['AyanokoyiBot-MD', 'Edge', '20.0.04'] : methodCodeQR ? ['AyanokoyiBot-MD', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "AyanokoyiJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './AyanokoyiJadiBot';
if (!existsSync(subBotDirectory)) {
console.log('🤍 Ayanokoyi Bot MD no tiene Sub-Bots vinculados.');
return;
}
...
const mainBotAuthFile = 'AyanokoyiSession';
try {
const mainBot = await connectionUpdate(mainBotAuthFile);
global.conns.push(mainBot);
console.log(chalk.bold.greenBright(`🤍 Ayanokoyi Bot MD conectado correctamente.`))
...
let prekey = []
let directorio = readdirSync("./AyanokoyiSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./AyanokoyiSession/${files}`)
})
} 
...
const directories = ['./AyanokoyiSession/', './AyanokoyiJadiBot/']
