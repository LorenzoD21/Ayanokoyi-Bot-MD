...
▶ ${usedPrefix + command} antilink2`, m, rcanal)
  ...
  conn.reply(m.chat, `*[ ℹ💢 ] La opción* \`\`\`${type}\`\`\` *fue* \`\`\`${isEnable ? 'activada' : 'desactivada'}\`\`\` *exitosamente para* ${isAll ? '*este* \`\`\`Bot\`\`\`' : isUser ? '' : '*este* \`\`\`chat\`\`\`'}`, m)
}
...
`.trim())
   ...
   m.reply(`*[ ℹ💢 ] La opción* \`\`\`${type}\`\`\` *fue* \`\`\`${isEnable ? 'activada' : 'desactivada'}\`\`\` *exitosamente para* ${isAll ? '*este* \`\`\`Bot\`\`\`' : isUser ? '' : '*este* \`\`\`chat\`\`\`'}`)
}
...
await conn.sendMessage(m.chat, { 
  ...
  buttons: [
    { 
      ...
      buttonText: { displayText: isEnable ? '💢 Off' : '🗯️ On' } 
    },
    { 
      buttonId: ".menu", 
